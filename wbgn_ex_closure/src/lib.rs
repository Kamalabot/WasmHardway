...
fn run() -> Result<(), JsValue>{
    ...
    // iterate over with array 
    // match idx for 0 and assign its value as string to the first_item
    ...
    // ensure by asserting that above process has been completed
...
fn setup_clock(window: &Window, document: &Document) -> Result<(), JsValue> {
    ...
    // there is a nested function definition
    fn update_time(current_time: &Element) {
        // this updates the page with the current time data
...
// `FnMut()`. Observe this function is not returning anything
