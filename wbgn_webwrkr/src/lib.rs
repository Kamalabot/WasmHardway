...
#[wasm_bindgen]
pub fn startup() {
    // multiple callbacks can interact with worker, so
    // using the ref_counted pointer
    let worker_handle = Rc::new(RefCell::new(Worker::new("./worker.js").unwrap()));
    console::log_1(&"Created web worker from inside wasm...".into());

    // pass the worker to function that sets up oninput callback
    setup_input_oninput_callback(worker_handle);
}

pub fn setup_input_oninput_callback(worker: Rc<RefCell<web_sys::Worker>>) {
    let document = web_sys::window().unwrap().document().unwrap();

    // in order keep the onmessage callback, we have to store it somewhere,
    // at the same time need to ensure memory is not leaked
